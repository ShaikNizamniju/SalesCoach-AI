...
const plans = [
  ...
  },
  {
    ...
    cta: "Contact us", href: "mailto:shaiknizamniju@gmail.com", highlight: false,
  },
];
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, alignItems: "start" }}>
          {plans.map((plan) => (
            <div key={plan.name} style={{
              background: plan.highlight ? "rgba(0,229,160,0.04)" : "var(--surface)",
              border: "1px solid " + (plan.highlight ? "rgba(0,229,160,0.3)" : "var(--border)"),
              borderRadius: 20, padding: "32px 28px", position: "relative",
              boxShadow: plan.highlight ? "0 0 40px rgba(0,229,160,0.08)" : "none",
            }}>
              {plan.highlight && (
                <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", background: "var(--accent)", color: "#000", fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: "0.08em", padding: "4px 14px", borderRadius: 20 }}>
                  MOST POPULAR
                </div>
              )}
