...
const stats = [
  ...
  { value: "3.2x", label: "higher close rate with structured coaching" },
];
...
const features = [
  { icon: "Target", title: "Call Score", desc: "0-100 rating across engagement, objection handling, and closing technique" },
  { icon: "Warning", title: "Lost Moment", desc: "Exact timestamp and transcript excerpt where the deal turned cold" },
  { icon: "Edit", title: "Better Script", desc: "Rewritten version of your critical moments that keeps prospects engaged" },
  { icon: "List", title: "Key Mistakes", desc: "Top 3 patterns across your call that cost you the deal" },
];
  ...
  return (
    <main style={{ position: "relative", zIndex: 1 }}>
      ...
      <section style={{ padding: "100px 40px 80px", maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "clamp(42px, 7vw, 72px)", lineHeight: 1.05, fontWeight: 800, letterSpacing: "-0.03em", marginBottom: 24 }}>
          Know exactly why<br />
          <span style={{ color: "var(--accent)" }}>you lost the deal.</span>
        </h1>
        <p style={{ fontSize: 18, color: "var(--muted2)", maxWidth: 560, margin: "0 auto 48px", lineHeight: 1.7, fontWeight: 300 }}>
          Upload your sales call. AI pinpoints the exact moment the prospect checked out and rewrites your script with a better approach.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <Link href="/analyze">
            <button className="btn-primary" style={{ fontSize: 15, padding: "14px 32px" }}>Analyze your first call - free</button>
          </Link>
          <Link href="/practice">
            <button className="btn-ghost" style={{ fontSize: 15, padding: "14px 32px" }}>Practice mode</button>
          </Link>
        </div>
        <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 20 }}>No account needed · Results in 30 seconds · Supports MP3, WAV, M4A</p>
      </section>

      <section style={{ borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px 40px", background: "var(--surface)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 40, maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          {stats.map((s) => (
            <div key={s.value}>
              <div style={{ fontFamily: "Syne, sans-serif", fontSize: 52, fontWeight: 800, color: "var(--accent)", letterSpacing: "-0.03em", lineHeight: 1 }}>{s.value}</div>
      ...
      <section style={{ padding: "80px 40px", maxWidth: 900, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          <div style={{ fontSize: 12, fontFamily: "Syne, sans-serif", fontWeight: 600, color: "var(--accent)", letterSpacing: "0.06em", marginBottom: 16 }}>HOW IT WORKS</div>
          <h2 style={{ fontSize: "clamp(28px, 4vw, 40px)", fontWeight: 800, letterSpacing: "-0.02em" }}>From recording to insight<br />in under 30 seconds</h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 24 }}>
          {steps.map((step) => (
            <div key={step.num} className="card">
              <div style={{ fontFamily: "Syne, sans-serif", fontSize: 11, fontWeight: 700, color: "var(--accent)", letterSpacing: "0.1em", marginBottom: 16 }}>{step.num}</div>
      ...
      <section style={{ padding: "0 40px 80px", maxWidth: 900, margin: "0 auto" }}>
        <div className="card" style={{ background: "var(--surface2)", borderColor: "var(--border2)", padding: "40px" }}>
          <h2 style={{ fontSize: 28, fontWeight: 800, letterSpacing: "-0.02em", marginBottom: 32 }}>What you get in every report</h2>
      ...
      <section style={{ padding: "80px 40px", textAlign: "center", borderTop: "1px solid var(--border)" }}>
        ...
        <p style={{ color: "var(--muted2)", fontSize: 16, marginBottom: 40 }}>First analysis is free. No sign-up required.</p>
        <Link href="/analyze">
          <button className="btn-primary" style={{ fontSize: 16, padding: "16px 40px", marginBottom: 40 }}>Upload your call now</button>
        </Link>
        <div style={{ maxWidth: 440, margin: "0 auto" }}>
          <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 14 }}>Get weekly sales coaching tips - free</p>
          <div style={{ display: "flex", gap: 10 }}>
            <input
              ...
              style={{ flex: 1, background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: 8, padding: "12px 16px", color: "var(--text)", fontSize: 14, outline: "none" }}
            />
            <button
      ...
      <footer style={{ borderTop: "1px solid var(--border)", padding: "24px 40px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <span style={{ fontSize: 12, color: "var(--muted)" }}>Built at OpenAI x Outskill AI Builders Hackathon 2025</span>
      </footer>
    </main>
  );
}
